...
vector<int> first_crossing_perCore_single_barrier_array(
...
py::array_t<int> first_crossing_single_barrier_array(
    ...
    for (int nn = 0; nn < nCPU; nn++) {
        ...
        int N_paths_core = N_paths / nCPU;
        N_paths_core += nn < (N_paths % nCPU);
        Cross_perCore[nn] = first_crossing_perCore_single_barrier_array(F_ij_reshaped, N_paths_core, N_Rfilt, delta_c);
        //#pragma atomic write
        //N_check += N_paths_core;
    }
