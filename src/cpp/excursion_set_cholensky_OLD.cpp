...
/*py::array_t<int> first_crossing_single_barrier_pycast(
    vector<double> &F_ij_reshaped, int N_paths, int N_Rfilt, double delta_c, int nCPU) {

    vector<int> std_vec = first_crossing_single_barrier_old(F_ij_reshaped, N_paths, N_Rfilt, delta_c, nCPU); //py::cast() py::array(vect_arr.size(), vect_arr.data());
...
/*py::array_t<int> first_crossing_from_multiplicity_func_pycast(vector<int> &NumCrossing) {

    int L = NumCrossing.size();

    vector<int> std_vec =  first_crossing_from_multiplicity_func(NumCrossing);

    py::array_t<int> np_arr = py::array_t<int>(L);
    py::buffer_info buf_np_arr = np_arr.request();
    int *ptr_np_arr = (int *) buf_np_arr.ptr;
...
py::array_t<double> first_crossing_from_multiplicity_func_pycast(vector<double> &NumCrossing) {

    int L = NumCrossing.size();

    vector<double> std_vec =  first_crossing_from_multiplicity_func(NumCrossing);

    py::array_t<double> np_arr = py::array_t<double>(L);
    py::buffer_info buf_np_arr = np_arr.request();
    int *ptr_np_arr = (int *) buf_np_arr.ptr;
...
PYBIND11_MODULE(excursion_set_cholensky, m) {
